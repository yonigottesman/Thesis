...
\paragraph {Memory}
A guest OS has no control over the hosts physical memory, just like any other process running on the system cannot control physical memory and only \comment{sees} its virtual address space.
%
...
\item
  ...
  allows multiple clients to access the same physical device while enforcing isolation at the hardware level. We discuss SR-IOV in section \comment{How to reference a sectino??} 
